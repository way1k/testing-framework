...
from tools.web_asserts import WebAsserts
...
class Browser:
    ...
    def __init__(
        self, browser: Literal["chrome", "local"], browser_version: str, bash_url: str, reqres_url: str, test_name: str
    ):

        if browser in ("local", "chrome", "firefox"):
            self.wd = self.driver_setup(browser=browser, version=browser_version, test_name=test_name)
        else:
    ...
    def driver_setup(browser: str, version: str, test_name: str):
        ...
        if capabilities["browserName"] == "local":
            ...
            chrome_remote_state_prefs = {
                "browser": {
                    "enabled_labs_experiments": ["treat-unsafe-downloads-as-active-content@2"],
                }
            }

            chrome_remote_options.add_experimental_option("localState", chrome_remote_state_prefs)
            ...
            driver = webdriver.Remote(
                command_executor=SELENOID_ENDPOINT, desired_capabilities=capabilities, options=chrome_remote_options
            )
